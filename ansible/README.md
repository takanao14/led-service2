...
Install the 30-second availability timer with `ansible-playbook playbooks/ops-led_service2_monitor.yaml --limit rpi3` from `ansible/`. Homelab must first provision the shared textfile directory and writer group. The restricted `led-monitor` account writes `led_service.prom` atomically; `led_service_active`, `led_service_grpc_success`, and `led_service_check_timestamp_seconds` distinguish failures from stale monitoring, and `led_service_restarts_total` exposes a crash loop, which leaves the unit active between restarts and is therefore invisible to the other three. The counter is omitted when systemd cannot be read, so a stale value never looks current. Set `led_service2_monitor_url` if the service uses another listen address. This probes gRPC validation, not display-worker progress, and never displays an image.
